...
// Ramses — drawn from scratch directly on Moses' construction: same 16-wide
// grid, one character per pixel, same K silhouette outline, same rounded
// organic head (no cube), same two-frame walk cycle with the identical leg
// timing, and the same built-in staff column on his right (gold instead of
// wood). Only the costume differs: white nemes with gold stripes, usekh
// collar, royal kilt and gold sandals. 24 rows tall so he towers over Moses
